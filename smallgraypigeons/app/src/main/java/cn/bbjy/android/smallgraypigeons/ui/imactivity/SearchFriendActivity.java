...
import cn.bbjy.android.smallgraypigeons.model.response.FriendInvitationResponse;
import cn.bbjy.android.smallgraypigeons.model.utils.AMUtils;
import cn.bbjy.android.smallgraypigeons.ui.ac.*;
...
//        super.doInBackground(requestCode, id);
    ...
    public void onSuccess(int requestCode, Object result) {
        if (result != null) {
            switch (requestCode) {
                case SEARCH_PHONE:
                    /* TODO final GetUserInfoByPhoneResponse userInfoByPhoneResponse = (GetUserInfoByPhoneResponse) result;
                                ...
                                DialogWithYesOrNoUtils.getInstance().showEditDialog(mContext, getString(R.string.add_text), getString(R.string.add_friend), new DialogWithYesOrNoUtils.DialogCallBack() {
                                    ...
                                    public void executeEditEvent(String editText) {
                                        ...
                                        if (TextUtils.isEmpty(editText)) {
                                            addFriendMessage = "我是" + getSharedPreferences("config", MODE_PRIVATE).getString(ConstantUtils.SEALTALK_LOGIN_NAME, "");
                                        }
    ...
    private boolean isFriendOrSelf(String id) {
        ...
        String selfPhoneNumber = sp.getString(ConstantUtils.SEALTALK_LOGING_PHONE, "");
        if (inputPhoneNumber != null) {
            if (inputPhoneNumber.equals(selfPhoneNumber)) {
                mFriend = new Friend(sp.getString(ConstantUtils.SEALTALK_LOGIN_ID, ""),
                        sp.getString(ConstantUtils.SEALTALK_LOGIN_NAME, ""),
                        Uri.parse(sp.getString(ConstantUtils.SEALTALK_LOGING_PORTRAIT, "")));
                return true;
            } else {
