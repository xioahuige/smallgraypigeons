...
import cn.bbjy.android.smallgraypigeons.model.response.AddGroupMemberResponse;
import cn.bbjy.android.smallgraypigeons.model.response.DeleteGroupMemberResponse;
...
import cn.bbjy.android.smallgraypigeons.utils.NToast;
    ...
    private void initData() {
        if (deleDisList != null && deleDisList.size() > 0) {
            for (int i = 0; i < deleDisList.size(); i++) {
                if (deleDisList.get(i).getUserId().contains(getSharedPreferences("config", MODE_PRIVATE).getString(ConstantUtils.SEALTALK_LOGIN_ID, ""))) {
                    continue;
                }
    ...
    private void fillSourceDataListForDeleteGroupMember() {
        if (deleteGroupMemberList != null && deleteGroupMemberList.size() > 0) {
            for (GroupMember deleteMember : deleteGroupMemberList) {
                if (deleteMember.getUserId().contains(getSharedPreferences("config", MODE_PRIVATE).getString(ConstantUtils.SEALTALK_LOGIN_ID, ""))) {
                    continue;
                }
    ...
    class StartDiscussionAdapter extends BaseAdapter implements SectionIndexer {
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                ...
                convertView = LayoutInflater.from(context).inflate(R.layout.item_start_disc, parent, false);
          ...
          // TODO  ImageLoader.getInstance().displayImage(portraitUri, viewHolder.mImageView, App.getOptions());
            return convertView;
        }
...
//            TextView tvUserId;
    ...
    public Object doInBackground(int requestCode, String id) throws HttpException {
       /* switch (requestCode) {
        ...
        } super.doInBackground(requestCode, id);*/
        return  null;
    }
